...
class GroupModeratorRepo:
    ...
    def get_all(self):
        db = self.db
        group_moderators = db.execute("SELECT * FROM group_moderator").fetchall()
    ...
    def get_by_moderator_username(self, moderator_username):
        db = self.db
        group_moderators = db.execute(
            "SELECT * FROM group_moderator WHERE moderator_username = ?",
            (moderator_username,),
        ).fetchall()
        return group_moderators
    ...
    def get_by_group_name_and_moderator_username(self, group_name, moderator_username):
        db = self.db
        group_moderators = db.execute(
            "SELECT * FROM group_moderator WHERE group_name = ? AND moderator_username = ?",
